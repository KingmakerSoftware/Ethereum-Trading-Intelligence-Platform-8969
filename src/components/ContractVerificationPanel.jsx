...
const ContractVerificationPanel=()=> {
  const {verifiedContracts,loading,error,verificationStats,autoVerificationQueue,isAutoVerifying,verifyContractAddress,fetchVerifiedContracts,deleteVerifiedContract,batchVerifyContracts,generateContractEtherscanUrl}=useContractVerification();
  ...
  const handleBatchVerify=async ()=> {
    const pendingDeployments=contractDeployments.filter(d=> d.verification_status==='pending' || !d.verification_status );
  ...
  // Format the verification time correctly with proper UTC to local timezone conversion
  const formatVerificationTime = (timestamp) => {
    if (!timestamp) return 'Unknown';
    
    try {
      // Create a Date object from the UTC timestamp
      // The timestamp from Supabase is in ISO format (UTC)
      ...
      // Use toLocaleString to convert to local timezone
      ...
      console.error('Error formatting verification time:', error, 'Timestamp:', timestamp);
      return 'Invalid date';
    }
  };
  ...
  const getVerificationStatusColor=(status)=> {
    switch (status) {
      ...
      case 'verifying': return 'text-yellow-400 bg-yellow-900/20';
      case 'failed': return 'text-red-400 bg-red-900/20';
      case 'no_contract': return 'text-gray-400 bg-gray-900/20';
  ...
  const getVerificationStatusIcon=(status)=> {
    switch (status) {
      ...
      case 'verifying': return FiRefreshCw;
      case 'failed': return FiXCircle;
      case 'no_contract': return FiAlertCircle;
        ...
        <div className="flex items-center justify-between mb-4"> 
          ...
          <div className="flex items-center space-x-2"> 
            ...
            > 
              ...
              <span> 
                {batchVerifying ? `Verifying ${batchProgress.current}/${batchProgress.total}` : `Verify ${verificationStats.pending} Pending`} 
              </span> 
            </button> 
          </div> 
        </div> 
            ...
            {contractDeployments.map((deployment)=> ( 
              ...
              > 
                <div className="flex items-center space-x-3 flex-1 min-w-0"> 
                  <div className="flex items-center space-x-2"> 
                    <SafeIcon 
                      ...
                      className={`${getVerificationStatusColor(deployment.verification_status).split(' ')[0]} ${deployment.verification_status==='verifying' ? 'animate-spin' : ''}`} 
                    /> 
                    <span 
                ...
                <div className="flex items-center space-x-2"> 
                  ...
                  {(deployment.verification_status==='failed' || deployment.verification_status==='pending' || !deployment.verification_status) && deployment.transaction_hash && ( 
                    <button 
                  ...
                  {(!deployment.verification_status || deployment.verification_status==='pending') && ( 
                    ...
                    > 
                      <SafeIcon icon={FiRefreshCw} className={verifying===deployment.id ? 'animate-spin' : ''} /> 
                    </button> 
                  )} 
                </div> 
              </div> 
                ...
                > 
                  <div className="flex items-center justify-between"> 
                      ...
                      <div className="flex-1 min-w-0"> 
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs"> 
                          ...
                          <div className="flex items-center space-x-1"> 
                            ...
                            <span className="text-gray-500" title={`Original UTC: ${contract.verified_at}`}> 
                              {formatVerificationTime(contract.verified_at)}
                            </span> 
                          </div> 
                        </div> 
                      </div> 
                    ...
                    <div className="flex items-center space-x-1 ml-2"> 
                      ...
                      > 
                        <SafeIcon icon={FiCopy} className={copiedAddress===contract.contract_address ? 'text-matrix-400' : ''} /> 
                      </button> 
                      <button 
